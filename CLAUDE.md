...
`src/lib/scrollState.ts` exports a plain mutable singleton object, deliberately **not** React state, because it's written and read ~60 times a second (by `ColorDirector`, the 3D scene, and `StoryBeats`). Putting this in `useState`/context would mean a React re-render per frame. Holds: `progress`/`smoothProgress`, `isFlooded`, `palette`, `activeProject` (`'none' | 'about' | 'surge' | 'laffy'`), `projectProgress`. Any new scroll-reactive feature should read/write this singleton rather than inventing a parallel scroll tracker.

## The 3D scene (`components/canvas/`)

- `BackgroundCanvas.tsx` — gates when/how the scene loads: waits for `requestIdleCallback` so first paint stays text-only, skips entirely under `prefers-reduced-motion`, and falls back to `MobileKnot` (a static SVG of the slope) if WebGL fails to initialize. Passes a `lite` flag when the viewport is under 768px, which `BackgroundScene` uses to cut geometry detail, cap `dpr`, and drop the `DebrisCorridor` layer — the goal is real 3D everywhere, just cheaper on phones/tablets, not a different fallback experience.
- `BackgroundScene.tsx` — the actual scene. The central shape is an ascending curve (`SlopePath` — "slope over intercept" embodied: shallow start, steepening rise, slight z undulation for depth) that **draws itself** as the page scrolls: the tube geometry is built once and only `geometry.setDrawRange` moves each frame, revealing more of the climb as the visitor descends (never empty — ~12% is visible at the top). There is deliberately no morphing/topology change anywhere (a previous circle-to-line morph produced persistent visual artifacts). A small tip marker rides the end of the drawn portion via `slopePath.getPoint(drawProgress)`. The slope reacts to `scrollState.activeProject`: blending material color toward the active project's palette color plus a small emissive lift. A `WAYPOINTS` array maps scroll progress → `{x, y, scale, opacity}` for gentle drift (opacity stays ≥0.75 — it must never read as disappearing). Also renders `DebrisCorridor` (instanced low-poly debris), `FinaleBloom` (particle burst at the slope's summit near the page bottom), and `LaffyBloom` (particle burst specifically while Laffy's story is active).

## Pinned scrollytelling (`StoryBeats.tsx`)

Each chapter's intro is a tall (`beats.length * 100vh`) wrapper with a `position: sticky` inner panel. `StoryBeats` takes a `StorySequence` (`data/projects.ts`), computes **local** scroll progress via `getBoundingClientRect` (not global page scroll), drives which "beat" word is shown (crossfaded with Framer Motion), and writes `scrollState.activeProject`/`projectProgress` — this is how the 3D scene and `SectionFrame` know a chapter is currently pinned, without any prop drilling. Three chapters use it: the personal About story (`ABOUT_STORY` — "Learn. Build. Break. Rebuild.", amber accent) and the two case studies (via `storyOf(caseStudy)`).
...
`src/data/projects.ts` defines each `CaseStudy` with `beats` (the story words scrubbed through in `StoryBeats`) and `accent: 'surge' | 'laffy'` (ties the card's hover-wash color and the 3D scene's color target to the same palette token), plus the `StorySequence` type, `storyOf()` helper, and `ABOUT_STORY` (the personal chapter's beats — one array, trivially editable). `src/data/socials.ts` is the footer link list. Adding a new case study means adding an entry here plus a new `surge`/`laffy`-style token in `palette.ts`'s `TokenName` if it needs its own color identity.

## Reveal-on-scroll vs. pinned story — two different systems

Most static content (paragraphs, footer blocks) uses the simple `.reveal` / `.is-visible` fade-up pattern, toggled once via `useReveal()`'s `IntersectionObserver` (`src/lib/useReveal.ts`). This is unrelated to `StoryBeats`'s pinned-scroll mechanic — don't conflate the two when adding new sections. Use `.reveal` for a one-time fade-in; use the `StoryBeats` pattern only for content that should scrub through multiple states as you scroll.
...
- rAF-driven effects (`Cursor`, `ColorDirector`, the 3D scene, `StoryBeats`, `SectionFrame`) mutate refs/DOM/CSS vars directly inside the loop rather than calling `setState` every frame; React state is only touched for discrete changes (e.g. a beat index changing).
